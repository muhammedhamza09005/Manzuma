...
def create_serial_number(self, _dict: dict) -> bool:
    ...
    while True:
        new_serial_number = fs.get_float(self, "Add/Remove a Serial Number? (no)", True)
...
def create_supplier(self) -> dict[str, Any]:
    if not fs.permisstion(self, "create-supplier"):
        fs.check_quit(self, "00", _clear_terminal=False)
    extra_str_ar = (
        "شركه صاله ثلاجة بىع لبىع تعبئه تحمىص اجود انواع مواد غذائى غذائىه شوكولاطه شوكولاته حلوى "
        "حلوىات بقولىات توابل مكسرات تمر تمور لحم لحوم مجمد مجمدات حفظ تبرىد عطر عطور تنظىف نظافه جمله قطاعى نقدا"
    )
    suppliers = fs.load_data(Path("data/suppliers.json"))
    while True:
        supplier_name = str(fs.get_str(self, "Supplier Name"))
...
def get_supplier(self) -> dict[str, Any]:
    ...
    _permisstion = fs.permisstion(self, "create-supplier", False)
    message = "Supplier Name/Number" + " (new supplier)" if _permisstion else str()
    while True:
        str_or_float = fs.get_str_or_float(self, message, _permisstion)
        if not str_or_float:
            return create_supplier(self)
...
def create_item(self, serial_number: int) -> dict[str, Any]:
    if not fs.permisstion(self, "create-item"):
        fs.check_quit(self, "00", _clear_terminal=False)
    while True:
        name = fs.get_str(self, "Item Name")
        name_already_exists = False
        ...
        if not name_already_exists:
            break
    pack = fs.get_float(self, "Pack (no pack)", True)
    imported_packs = None
    if pack is not None:
        imported_packs = float(fs.get_float(self, "Imported Packs"))
        imported = imported_packs * pack
    else:
        imported = float(fs.get_float(self, "Imported Items"))
    in_stock = fs.get_float(self, f"In-Stock Items ({imported})", True)
    in_stock = in_stock if in_stock is not None else imported
    purchase_pack_price = None
    if pack is not None:
        purchase_pack_price = float(fs.get_float(self, "Purchase pack Price"))
        purchase_price = purchase_pack_price / pack
    else:  # self.settings.profit, self.settings.divide
        purchase_price = float(fs.get_float(self, "Purchase Price"))
    profit = float(self.calculate_profit(purchase_price))
    sell_price = fs.get_float(self, f"Sell Price ({purchase_price} -> {profit})", True)
    ...
    if purchase_pack_price:
        ...
        sell_pack_price = fs.get_float(self, f"Sell Pack Price ({purchase_pack_price} -> {pack_profit})", True)
...
def get_item(self) -> tuple[dict | None, str]:
    serial_number = int()
    _permisstion = fs.permisstion(self, "create-purchase-item", False)
    message = "Item Name/Serial Number" + " (*)" if _permisstion else str()
    str_or_float = fs.get_str_or_float(self, message, _permisstion)
    if not str_or_float:
    ...
    if type(found_items) is list and len(found_items) == 1:
        ...
        if is_delete and fs.permisstion(self, "delete-purchase-item") and not fs.get_str(self, message, True):
            return delete_item(self, found_items[0]), str()
        return found_items[0], "get"
    if serial_number and _permisstion:
        return create_item(self, serial_number), "create"
    return None, str()


def update_item(self, item: dict) -> dict[str, Any]:
    if not fs.permisstion(self, "update-item"):
        fs.check_quit(self, "00", _clear_terminal=False)
    pack = item["pack"]
    imported_packs = None
    if pack is not None:
        imported_packs = fs.get_float(self, "Imported Packs")
        imported = imported_packs * pack
    else:
        imported = fs.get_float(self, "Imported Items")
    in_stock = fs.get_float(self, f"In-Stock Items ({imported})", True) or imported
    purchase_pack_price = None
    if pack is not None:
        purchase_pack_price = (
            fs.get_float(self, f"Purchase pack Price ({item["purchase-pack-price"]})", True) or item["purchase-pack-price"]
        )
        purchase_price = purchase_pack_price / pack
    else:
        purchase_price = fs.get_float(self, f"Purchase Price ({item["purchase-price"]})", True) or item["purchase-price"]
    if purchase_price != item["purchase-price"]:
        profit = self.calculate_profit(purchase_price)
        sell_price = fs.get_float(self, f"Sell Price ({purchase_price} -> {profit})", True) or profit
        pack_profit = self.calculate_profit(purchase_pack_price)
        sell_pack_price = (
            fs.get_float(self, f"Sell Pack Price ({purchase_pack_price} -> {pack_profit})", True) or pack_profit
        )
    else:
        sell_price = fs.get_float(self, f"Sell Price ({item["sell-price"]})", True) or item["sell-price"]
        sell_pack_price = fs.get_float(self, f"Sell Pack Price ({item["sell-pack-price"]})", True) or item["sell-pack-price"]
    _item = {
...
def add_to_item(self, item: dict) -> dict[str, Any]:
    ...
    if pack is not None:
        ...
        imported = fs.get_float(self, "Imported Items")
    in_stock = (fs.get_float(self, f"In-Stock Items ({imported})", True) or imported) + item["in-stock"]
    if imported_packs:
...
def create_invoice(self) -> dict[str, Any]:
    if not fs.permisstion(self, "create-purchase-invoice"):
        fs.check_quit(self, "00", _clear_terminal=False)
    supplier_invoice_number = fs.get_float(self, "Supplier Invoice Number (no number)", True)
...
def get_invoice(self) -> dict | None:
    ...
    _permisstion = fs.permisstion(self, "create-purchase-invoice", False)
    ...
    if len(invoices) == 1:
        if (
            is_delete
            and fs.permisstion(self, "delete-purchase-invoice")
            and not fs.get_str(self, f"Are you sure you want to DELETE purchase invoice: {invoice_number}? (Yes)", True)
        ):
